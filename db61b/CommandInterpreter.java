...
class CommandInterpreter {
    ...
    Table selectClause() {
        ...
        if (_input.nextIf("where")) {
            if (table2 != null) {
                conditionals = conditionClause(table, table2);
            } else {
                conditionals = conditionClause(table);
            }
        }
        _input.nextIf(";");
        if (table2 != null) {
            return table.select(table2, colNames, conditionals);
        } else {
            return table.select(colNames, conditionals);
        }
    ...
    ArrayList<Condition> conditionClause(Table... tables) {
        ...
        Condition toAdd = condition(tables);
        conditions.add(toAdd);
        while (_input.nextIf("and")) {
            toAdd = condition(tables);
            conditions.add(toAdd);
        }
        return conditions;
    }
    ...
    Condition condition(Table... tables) {
        String nameOfCol = columnName();
        Column compareCol = new Column(nameOfCol, tables);
        String relation = _input.next(RELATION);
        String compareVal;
        Column compare;
        if (_input.nextIs(LITERAL)) {
            compareVal = literal();
            return new Condition(compareCol, relation, compareVal);
        } else {
            compareVal = columnName();
            compare = new Column(compareVal, tables);
            return new Condition(compareCol, relation, compare);
        }
